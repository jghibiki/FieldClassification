...
k = 0
for image_no in xrange(1,2):
    im = gdal.Open("input_test_%s.tif" % image_no)
    ...
    for i in xrange(0, source_w, out_w):
        for j in xrange(0, source_h, out_h):
            ...
            if not np.argmax(count) == 0 :
                r.load()
                r.save("test_raw_images/IMG-R-%008d.png" % k)

                g = image_g.crop((i, j, i+out_h, j+out_w))
                g.load()
                g.save("test_raw_images/IMG-G-%008d.png" % k)

                b = image_b.crop((i, j, i+out_h, j+out_w))
                b.load()
                b.save("test_raw_images/IMG-B-%008d.png" % k)

                a = image_a.crop((i, j, i+out_h, j+out_w))
                a.load()
                a.save("test_raw_images/IMG-A-%008d.png" % k)

                label_img = label.crop((i, j, i+(out_h), j+(out_w)))
                label_img.load()
                label_img.save("test_raw_images/LBL-%008d.png" % k)

                b = Image.merge("RGB", [r, g, b])
                b.save("test_raw_images/IMG-%008d-visible.png" % k)
